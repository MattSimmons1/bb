...
func Parse(text string, funcs ...map[string]interface{}) (map[string]*Tree, error) {
...
func New(funcs ...map[string]interface{}) *Tree {
	return &Tree{
		Name:  "bb",
		funcs: funcs,
	}
...
func (t *Tree) Parse(text string, treeSet map[string]*Tree, funcs ...map[string]interface{}) (tree *Tree, err error) {
	defer t.recover(&err)
	t.ParseName = t.Name
	t.startParse(funcs, lex(text), treeSet)
	t.text = text
	t.parse()
	t.add()
	t.stopParse()
...
func (t *Tree) parse() {
  verbose_print("parse")
	...
	for t.peek().typ != itemEOF {
		...
		//		newT := New() // name will be updated once we know it.
		//		newT.text = t.text
...
func (t *Tree) textOrAction() Node {
	// TODO: we don't care about text - remove this step
...
func (t *Tree) blockControl() Node {
	...
	block := New() // name will be updated once we know it.
	block.text = t.text
